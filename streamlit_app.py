...
# Load the Keras model (model file must be in the repo with this path)
@st.cache_resource(show_spinner=False)
def load_model():
    return tf.keras.models.load_model("animal_classification_model.keras")

model = load_model()

# Define animal classes (correct spelling)
classes = ['elephant', 'cheetah', 'wild boar']
...
def predict_and_draw(image):
    ...
    # Debug info for prediction output shape and values
    ...
    # Parsing prediction based on output size: either [class_probs] or [class_probs + bbox]
    if prediction.shape[1] == len(classes):
        class_probs = prediction[0]
        bbox = None
    elif prediction.shape[1] == len(classes) + 4:
        class_probs = prediction[0][:len(classes)]
        bbox = prediction[0][len(classes):len(classes)+4]
    else:
        st.error(f"Unexpected prediction output shape: {prediction.shape}")
        return image
...
# Use camera input or allow upload
